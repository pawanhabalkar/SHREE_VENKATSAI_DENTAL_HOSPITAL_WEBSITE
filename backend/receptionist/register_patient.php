...
if (!preg_match("/^[0-9]{10}$/", $mobile)) {
    echo json_encode(["error" => "invalid_mobile"]);
    exit();
}
...
try {
    ...
    // 2. Generate the next MRN, e.g. SVMSDH-000001. Row-locked (see
    //    sequence.php) so two walk-ins registered at once can't
    //    collide on the same MRN.
    ...
    $knownErrors = ["database_error", "email_already_registered"];
    $error = in_array($e->getMessage(), $knownErrors, true) ? $e->getMessage() : "database_error";
