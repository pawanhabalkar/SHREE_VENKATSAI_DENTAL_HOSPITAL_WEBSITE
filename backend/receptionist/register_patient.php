...
// Patients registered at the desk don't set their own password.
// Generate a random one now; a receptionist/admin can reset it later
// if the patient wants to use the online patient portal.
...
try {
    ...
    $countResult = mysqli_query($conn, "SELECT COUNT(*) AS total FROM patients");
    $total = mysqli_fetch_assoc($countResult)["total"];
    $nextNumber = $total + 1;
    $mrn = "SVMSDH-" . str_pad($nextNumber, 6, "0", STR_PAD_LEFT);
